...
public class MultiDownloader extends AsyncTask<MultiDownloadParams, int[], DownloadResult[]>{
    private MultiDownloadParams mParam;
    private AtomicBoolean mAbort = new AtomicBoolean(false);
    private DownloadResult[] mDownloadResults;

    protected DownloadResult[] doInBackground(MultiDownloadParams... params) {
        mParam = params[0];

        mDownloadResults = new DownloadResult[mParam.srcURLs.length];

        try {
            this.download(mParam);
            mParam.onTaskCompleted.onTaskCompleted(mDownloadResults);
        } catch (Exception ex) {
            ex.printStackTrace();
            mParam.onTaskCompleted.onTaskCompleted(mDownloadResults);
    ...
    private void download(MultiDownloadParams downloadParams) {
...
//                mParam.onDownloadBegin.onDownloadBegin(statusCode, lengthOfFile, headersFlat);
                ...
                output = new FileOutputStream(downloadParams.destFiles[i]);
...
//                    }
                ...
                try {
                    ...
                    Log.d("RNFS MultiDownloader", "Exception in finally");
                    e.printStackTrace();
                }
            }
        }
